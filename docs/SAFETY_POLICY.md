...
- Scheduled Task는 22:55에 `shutdown.exe -s -t 300`을 실행해 23:00 종료를 목표로 한다.
- 점검은 task 조건과 취소 shortcut 동일성을 모두 확인한다. task가 정상이어도 취소 shortcut이 없거나 원본과 다르면 `WARNING`을 유지한다.
- 한국어 Windows의 `schtasks` 오전/오후 시간 출력은 24시간 기준으로 파싱해야 한다.

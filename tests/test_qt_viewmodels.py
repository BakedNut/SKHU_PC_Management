...
class FakeLoadPcInfo:
    def execute(self) -> PcInfo:
        return PcInfo(
            ...
            memory_gb=16,
            memory_modules=[
                MemoryModuleInfo("Slot 1", capacity_gb=8, memory_type="DDR4", speed_mhz=3200),
                MemoryModuleInfo("Slot 2", capacity_gb=8, memory_type="DDR4", speed_mhz=3200),
            ],
            gpu_name="NVIDIA RTX",
            gpu_memory="8GB",
...
def test_pc_info_viewmodel_refresh_updates_rows() -> None:
    ...
    assert view_model.ram == "16GB (2개: DDR4-3200 8GB x2)"
    assert view_model.gpu == "NVIDIA RTX"
    assert view_model.gpu_memory == "8GB"
    assert view_model.ipv4_address == "192.168.0.10"
...
def test_pc_info_viewmodel_formats_ram_module_groups() -> None:
    assert _format_ram(
        PcInfo(
            computer_name="PC01",
            user_name="student",
    ...
    ) == "24GB (2개: DDR5-5600 16GB x1, DDR4-3200 8GB x1)"

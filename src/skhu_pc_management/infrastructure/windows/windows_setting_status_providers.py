...
class TaskbarLayoutStatusProvider:
    ...
    def check(self, setting_id: str) -> SettingStatus:
        with EnvProfiler("SKHU_PC_MANAGEMENT_PROFILE_TABS").step("taskbar_layout_status.check"):
            label = "작업표시줄 아이콘 설정"
            try:
                ...
                issues = _target_taskbar_shortcut_issues(source_names, target_dir)
            except Exception as exc:
            ...
            detail = (
                ...
                "shortcut target 상세 확인은 기본 상태 확인에서 생략됩니다. "
...
class PasswordExpirationStatusProvider:
    ...
    def check(self, setting_id: str) -> SettingStatus:
        with EnvProfiler("SKHU_PC_MANAGEMENT_PROFILE_TABS").step("password_expiration.check"):
            ...
            if max_password_age_unlimited is True:
                detail = (
                    "사용자 계정 암호 만료 정책이 비활성화되어 있습니다. "
                    "최대 암호 사용 기간: 무제한 / 개별 사용자 플래그 상세 확인은 생략됨"
                )
                return SettingStatus(
                    setting_id=setting_id,
                    label=label,
